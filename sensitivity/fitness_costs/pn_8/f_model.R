...
model <- function(t, x, parms) {
  with(as.list(parms),{
    dx <- numeric()
      ...
      dx[1] <- # susceptible
        ( 
          - x[1]*c[1]*sum(rho[1,]*beta[1,]*c(x[3:4])/N) # new infections
          - x[1]*c[1]*sum(rho[1,]*beta[1,]*(1-cost_b)*c(x[5:6])/N) # new infections
          + sum(v*x[c(3)]) # spontaneous clearance
          + sum((v/(1-cost_v))*x[c(5)]) # spontaneous clearance
    ...
    dx[2] <- # susceptible
      ( 
        - x[2]*c[2]*sum(rho[2,]*beta[2,]*c(x[3:4])/N) # new infections
        - x[2]*c[2]*sum(rho[2,]*beta[2,]*(1-cost_b)*c(x[5:6])/N) # new infections
        + sum(v*x[c(4)]) # spontaneous clearance
        + sum((v/(1-cost_v))*x[c(6)]) # spontaneous clearance
    ...
    dx[4] <- # infected WT
      ...
      dx[5] <- # infected A resistant
        (
          x[1]*c[1]*sum(rho[1,]*beta[1,]*(1-cost_b)*x[5:6]/N) 
          - (v/(1-cost_v))*x[5] 
      ...
      dx[6] <- # infected A resistant
        (
          x[2]*c[2]*sum(rho[2,]*beta[2,]*(1-cost_b)*x[5:6]/N) 
          - (v/(1-cost_v))*x[6] 
