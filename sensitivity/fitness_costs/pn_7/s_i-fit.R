...
for(i in 2:nrow(res.out)){
  eq.temp[i-1,] <- abs(res.out[i,2:ncol(res.out)]-res.out[i-1,2:ncol(res.out)])<10^-10
}
eq.row <- which(abs(rowSums(eq.temp)-ncol(eq.temp))<10^-10)-1 # gives all rows in equilibrium (rows give row number in res.out)
eq.sw <- eq.row[which((eq.row-which(abs(res.out[,1]-swtime)<10^-10))>0)] # gives all equilibria indices after switch
...
for (n in 1:3){
  ...
  if("try-error" %in% class(triedfit)){
    costcomp[cci,l,m,n,1] <- NA
    costcomp[cci,l,m,n,2] <- NA
    costcomp[cci,l,m,n,3] <- NA
    nofit <- c(nofit, paste(k,l,m,n, sep=" "))
  }else if(summary(log.ss)$coef[1]<0.99){
    costcomp[cci,l,m,n,1] <- NA
    costcomp[cci,l,m,n,2] <- NA
    costcomp[cci,l,m,n,3] <- NA
    corrfit <- c(nofit, paste(k,l,m,n, sep=" "))
  }else{
    costcomp[cci,l,m,n,1] <- 1/summary(log.ss)$coef[3] # b
    costcomp[cci,l,m,n,2] <- summary(log.ss)$coef[1] # c = asymptote
    costcomp[cci,l,m,n,3] <- exp(summary(log.ss)$coef[2]/summary(log.ss)$coef[3]) # a
  }
}