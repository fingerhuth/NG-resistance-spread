...
load(paste("output/ros_", pop, "-", seed, "-", cond, ".data", sep=""))
...
pdf(file=paste("plots/fig_", pop, "-", seed, "-", cond, ".pdf", sep=""), width=21, height=35)
...
#   else{
#     polygon(c(dens.prior$x, dens.prior$x[1]), c(dens.prior$y, dens.prior$y[1]), col=rgb(0,110,255,100,maxColorValue=255), border=NA)
#   }
  close.screen(n=j+12)
}
